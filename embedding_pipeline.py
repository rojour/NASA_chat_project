...
class ChromaEmbeddingPipelineTextOnly:
    ...
    def __init__(self, 
        ...
        """
        # Initialize OpenAI client (using Vocareum proxy for educational access)
        self.openai_client = OpenAI(
            api_key=openai_api_key,
            base_url="https://openai.vocareum.com/v1"
        )

        # Store configuration parameters for use in other methods
        ...
        # Create OpenAI embedding function for ChromaDB (with Vocareum base URL)
    ...
    def chunk_text(self, text: str, metadata: Dict[str, Any]) -> List[Tuple[str, Dict[str, Any]]]:
        ...
        while start < len(text):
            ...
            # If not at the end, try to break at sentence boundaries
            if end < len(text):
    ...
    def generate_document_id(self, file_path: Path, metadata: Dict[str, Any]) -> str:
        ...
        Returns:
            Consistent ID in format: mission_source_chunk_0001
        """
        # Get components for ID
        mission = metadata.get('mission', 'unknown')
        source = metadata.get('source', file_path.stem)
        chunk_index = metadata.get('chunk_index', 0)

        # Create consistent ID format: mission_source_chunk_0001
    ...
    def add_documents_to_collection(self, documents: List[Tuple[str, Dict[str, Any]]],
        ...
        # Handle replace mode - delete existing documents from this file first
        ...
        for i in range(0, len(documents), batch_size):
            ...
            for text, metadata in batch:
                ...
                # Add to batch for new documents (or replace mode)
    ...
    def process_all_text_data(self, base_path: str, update_mode: str = 'skip') -> Dict[str, int]:
        ...
        for file_path in files:
            try:
                ...
                # Process file to get chunks with metadata
    ...
    def get_collection_info(self) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with collection name, document count, and metadata
        """
        try:
            # Get collection metadata (if any was set during creation)
